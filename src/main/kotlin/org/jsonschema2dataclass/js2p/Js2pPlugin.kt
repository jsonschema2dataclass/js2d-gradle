...
private fun setTaskConfiguration(
    project: Project,
    task: Js2pGenerationTask,
    configuration: Js2pConfiguration,
    defaults: Js2pConfiguration
) {
    task.source.setFrom(
        if (configuration.source.isEmpty) {
            defaults.source
    ...
    task.constructorsRequiredPropertiesOnly = maybeDefault(
        project,
        configuration.constructorsRequiredPropertiesOnly,
        defaults.constructorsRequiredPropertiesOnly
    )
    ...
    task.customDateTimePattern =
        maybeDefault(project, configuration.customDateTimePattern, defaults.customDateTimePattern)
    ...
    task.includeAdditionalProperties =
        maybeDefault(project, configuration.includeAdditionalProperties, defaults.includeAdditionalProperties)
    task.includeAllPropertiesConstructor =
        maybeDefault(project, configuration.includeAllPropertiesConstructor, defaults.includeAllPropertiesConstructor)
    task.includeConstructorPropertiesAnnotation = maybeDefault(
        project,
        configuration.includeConstructorPropertiesAnnotation,
        defaults.includeConstructorPropertiesAnnotation
    )
    task.includeConstructors = maybeDefault(project, configuration.includeConstructors, defaults.includeConstructors)
    task.includeCopyConstructor =
        maybeDefault(project, configuration.includeCopyConstructor, defaults.includeCopyConstructor)
    task.includeDynamicAccessors =
        maybeDefault(project, configuration.includeDynamicAccessors, defaults.includeDynamicAccessors)
    task.includeDynamicBuilders =
        maybeDefault(project, configuration.includeDynamicBuilders, defaults.includeDynamicBuilders)
    task.includeDynamicGetters =
        maybeDefault(project, configuration.includeDynamicGetters, defaults.includeDynamicGetters)
    task.includeDynamicSetters =
        maybeDefault(project, configuration.includeDynamicSetters, defaults.includeDynamicSetters)
    task.includeGeneratedAnnotation =
        maybeDefault(project, configuration.includeGeneratedAnnotation, defaults.includeGeneratedAnnotation)
    task.includeGetters = maybeDefault(project, configuration.includeGetters, defaults.includeGetters)
    task.includeHashcodeAndEquals =
        maybeDefault(project, configuration.includeHashcodeAndEquals, defaults.includeHashcodeAndEquals)
    task.includeJsr303Annotations =
        maybeDefault(project, configuration.includeJsr303Annotations, defaults.includeJsr303Annotations)
    task.includeJsr305Annotations =
        maybeDefault(project, configuration.includeJsr305Annotations, defaults.includeJsr305Annotations)
    task.includeRequiredPropertiesConstructor = maybeDefault(
        project,
        configuration.includeRequiredPropertiesConstructor,
        defaults.includeRequiredPropertiesConstructor
    )
    ...
    task.initializeCollections =
        maybeDefault(project, configuration.initializeCollections, defaults.initializeCollections)
    ...
    task.propertyWordDelimiters =
        maybeDefault(project, configuration.propertyWordDelimiters, defaults.propertyWordDelimiters)
    task.refFragmentPathDelimiters =
        maybeDefault(project, configuration.refFragmentPathDelimiters, defaults.refFragmentPathDelimiters)
    ...
    task.useInnerClassBuilders =
        maybeDefault(project, configuration.useInnerClassBuilders, defaults.useInnerClassBuilders)
    ...
    task.useOptionalForGetters =
        maybeDefault(project, configuration.useOptionalForGetters, defaults.useOptionalForGetters)
