...
class Js2pConfigConversionTest {
    ...
    fun nullAnnotationStyleDefaultsToJackson() {
        ...
        Assertions.assertEquals(AnnotationStyle.JACKSON, config.annotationStyle)
    }
    ...
    fun explicitAnnotationStylePreserved() {
        ...
        Assertions.assertEquals(AnnotationStyle.GSON, config.annotationStyle)
    }
    ...
    fun invalidCustomAnnotatorThrowsException() {
        ...
        val exception = Assertions.assertThrows(GradleScriptException::class.java) {
            config.customAnnotator // Trigger lazy class loading
        }
        Assertions.assertTrue(exception.message?.contains("Unable to find class") == true)
    }
    ...
    fun nullCustomAnnotatorUsesDefault() {
        ...
        Assertions.assertEquals("org.jsonschema2pojo.NoopAnnotator", config.customAnnotator.name)
    }

    private fun createJs2pConfig(
        annotationStyle: String? = null,
        customAnnotatorClass: String? = null,
    ): Js2pConfig = Js2pConfig(
